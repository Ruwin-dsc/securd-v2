...
module.exports = {
    ...
    async run(client, webhook) {
        if (webhook.action_type !== 52) return;
        ...
        const allItemPermissions = client.managers.permissionManager.filter((permissions, key) => key.split("-")[1] === guild.id && permissions.has("updateWebhooks"));
        let bypass = false;
        const log = (await guild.fetchAuditLogs({ type: 52, limit: 1 })).entries.first();
        ...
        if (bypass) return;
        (await guild.fetchWebhooks()).find((w) => w.channelId === webhook.target_id && w.name === webhook.changes[0].old_value.name).delete("Securd - Anti-WebhookUpdate").catch(() => { });
        client.util.punish(guild, executorMember, config.punish, "Securd - Anti-WebhookUpdate", "Failed to punish the user for updating a webhook", "Successfully punished the user for updating a webhook")
    }
}